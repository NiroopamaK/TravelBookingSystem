...
exports.getProfile = async (req, res) => {
    ...
    try {
        ...
        res.render('editProfile', { user: rows[0] , token: token})
...
exports.updateProfile = async (req, res) => {
    ...
    try {
        ...
        if (password && password.length > 0) {
            ...
            await db.query(`
                UPDATE users
                SET first_name=?, last_name=?, address=?, telephone=?, password=?
                WHERE user_id=?
            `, [first_name, last_name, address, telephone, hashedPassword, userId])
